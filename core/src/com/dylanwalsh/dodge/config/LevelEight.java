...
public class LevelEight extends LevelConfig {
    ...
    public LevelEight() {
        ...
        aliens.put("min_velocity", Gdx.graphics.getWidth()/7.3f);//change - reduced range
        ...
        //Level 8 change every attribute
        alien.put("fire_rate", 1.8f); //change - faster firing
        alien.put("bullet_speed", 1.18f); //change - faster bullets
        alien.put("bullet_size", Gdx.graphics.getWidth()/42f); //change - bigger bullets
        alien.put("fire_delay", 1); //change - cant change
        alien.put("fire_probability", 2); //change - shouldn't change yet
        alien.put("velocity_change_probability", 4); //change - reduced probability

        background.put("stars_per_spawn", 4); //change - more stars
        ...
        background.put("min_star_speed", Gdx.graphics.getWidth()/7.5f);

        debris.put("spawn_rate", 0.7f);
        debris.put("spawn_delay", 2); //round wait
        debris.put("spawn_count", 11); //change - more debris
        debris.put("max_vel", Gdx.graphics.getWidth() / 6.5f);
        debris.put("min_vel", Gdx.graphics.getWidth() / 8.5f);

        debrisItem.put("fade_interval", 0.14f);//change - quicker fade
        ...
        gammaRays.put("min_length", 20); //change - reduce range
    }
